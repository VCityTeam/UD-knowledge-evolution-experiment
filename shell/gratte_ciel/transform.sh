...
docker run --name "annotate_graph-relational" -v "$PWD:/data" vcity/quads-creator "/data/quads/relational" "/data/triples" "*" relational BSBM
...
do
    file=$(basename "$file")
    docker run --name "relational_annotate_graph-$(basename "$file")" -v "$PWD:/data" vcity/quads-creator "/data/quads/theoretical" "/data/triples" "$file" relational Metadata
done

docker run --name "relational_annotate_graph-workspace" -v "$PWD:/data" vcity/quads-creator "/data/quads/theoretical" "/data/triples" GratteCiel_2009_2018_Workspace.ttl relational Metadata
docker ps --filter name=relational_annotate_graph-* -aq | xargs docker stop | xargs docker rm

find . -type f -name "*split.ttl" -print0 | while IFS= read -r -d '' file
do
    file=$(basename "$file")
    docker run --name "theoretical_annotate_graph-$(basename "$file")" -v "$PWD:/data" vcity/quads-creator "/data/quads/theoretical" "/data/triples" "$file" theoretical Grand-Lyon
done
...
#do
#    file=$(basename "$file")
#    docker run --name "annotate_graph-$(basename "$file")" -v "$PWD:/data" vcity/quads-creator "/data/quads/relational" "/data/triples" "$file" nt  relational Grand-Lyon
#done
